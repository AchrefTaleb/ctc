            ...
            <div class="statbox widget box box-shadow">
                ...
                <table id="staff-table" class="multi-table table table-striped style-3 table-bordered table-hover non-hover" style="width:100%">
                    ...
                    <tr>
                        ...
                        <td> @if($client->subscription) {{ $client->subscription->created_at->format('m-y') ?? '' }} / {{ $client->subscription->commitment ?? '' }}  / {{ $client->subscription->plan->name }}@else  --- @endif </td>
                        <td> @if($client->contract) <span class="shadow-none badge badge-primary">contract signeé</span>   @else
                                <form>
                                    <input type="hidden" name="user" value="{{ $client->id }}">
                                    <button type="submit" class="btn-info">Activer le contract</button>
                                </form>
                            @endif</td>

                ...
                <div class="widget-header">
                    <div class="row">
                        <div class="col-xl-12 col-md-12 col-sm-12 col-12 inline">
                          ...
                          @if($opening != 'illimited')  <div style="margin-bottom: 0;margin-top: 5px;" class="float-right alert alert-warning mb-4" role="alert"> <button type="button" class="close" data-dismiss="alert" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x close" data-dismiss="alert"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button> Il vous reste {{ $opening  }} overtures durant ce mois !</div>@endif
                        ...
                        <table id="staff-table" class="multi-table table table-striped style-3 table-bordered table-hover non-hover" style="width:100%">
                            ...
                            @foreach($mails as $mail)
                                <tr @if(!$mail->open) style="background-color: #f1f2f3 !important;" @endif>
                                    ...
                                    <td>{{ $mail->code }}</td>
                                    <td>{{ $mail->from ?? "-----" }}</td>
                                    <td>{{ $mail->client->id }} - {{ $mail->client->name.' '.$mail->client->last_name }}</td>
                                    <td>@if($mail->type == "mail") Courrier  @else Colis  @endif</td>                                    <td>{{ $mail->category->name ?? "-----" }}</td>
